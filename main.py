# ================================
# main.py — RAILWAY VERSION
# DuckDB + 1GB Parquet (no full in-memory table)
# ================================
...
LOCAL_PATH = "/tmp/dataset.parquet"  # Railway ephemeral disk is fine

# =========================================================
# DOWNLOAD DATASET ONCE (PER CONTAINER START)
# =========================================================
def ensure_dataset():
...
# =========================================================
# DUCKDB CONNECTION + VIEW (NO FULL COPY IN MEMORY)
# =========================================================
# Use an in-memory DB but read directly from the parquet file.
# This avoids materializing the full 1GB into a DuckDB table.
...
# =========================================================
# PRELOAD DROPDOWN VALUES (SMALL QUERIES)
# =========================================================
boroughs = (
...
_years_raw = q("SELECT DISTINCT crash_year FROM collisions ORDER BY crash_year")[
    "crash_year"
].tolist()
years = [int(y) for y in _years_raw if y is not None]

vehicles = q(
    "SELECT DISTINCT vehicle_category FROM collisions ORDER BY vehicle_category"
)["vehicle_category"].tolist()

factors = q(
    "SELECT DISTINCT contributing_factor_combined FROM collisions ORDER BY contributing_factor_combined"
)["contributing_factor_combined"].tolist()

injuries = q(
    "SELECT DISTINCT person_injury FROM collisions ORDER BY person_injury"
)["person_injury"].tolist()
...
def build_where(borough, year, vehicle, factor, injury, search):
    ...
    if factor:
        filters.append(
            "contributing_factor_combined IN (" + ",".join(repr(f) for f in factor) + ")"
        )
    if injury:
...
# =========================================================
# LIMIT HEAVY QUERIES (memory & performance)
# =========================================================
def where_light(where_clause: str, year_filter):
    # If user already filters by year, respect it fully
    if year_filter:
        return where_clause
...
def update(n, borough, year, vehicle, factor, injury, search):
    ...
    if df8.empty:
        ...
        pivot = (
            df8.pivot(index="vehicle_category", columns="hour", values="avg_sev")
            .fillna(0)
        )
...
if __name__ == "__main__":
    # Railway usually sets PORT env var
    port = int(os.environ.get("PORT", 8050))
    print(f"🚀 Starting Dash on 0.0.0.0:{port}")
