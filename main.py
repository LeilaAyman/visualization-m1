...
LOCAL_PATH = r"C:\Users\noura\Documents\visualization-m1\final_cleaned_final.parquet"
...
# ============================================================
# WHERE BUILDER — MULTI-COLUMN SEARCH
# ============================================================
def build_where(borough, year, vehicle, factor, injury, query):
    ...
    if query and query.strip() != "":
        ...
        filters.append(f"""
            (
                LOWER(borough) LIKE '%{qtext}%'
                OR LOWER(on_street_name) LIKE '%{qtext}%'
                OR LOWER(vehicle_category) LIKE '%{qtext}%'
                OR LOWER(contributing_factor_combined) LIKE '%{qtext}%'
                OR LOWER(person_type) LIKE '%{qtext}%'
                OR LOWER(person_injury) LIKE '%{qtext}%'
                OR LOWER(CAST(crash_year AS VARCHAR)) LIKE '%{qtext}%'
            )
        """)
...
def update(_, borough, year, vehicle, factor, injury, query):
    ...
    hour_filter = "AND hour BETWEEN 0 AND 23" if where else "WHERE hour BETWEEN 0 AND 23"
    ...
    # ============================================================
    # 5 — Injured / Killed / Severity by Vehicle Category
    # ============================================================
    df5 = q(f"""
        SELECT 
            vehicle_category,
