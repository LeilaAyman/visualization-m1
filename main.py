...
# =========================================================
# DOWNLOAD DATASET ONCE INTO /tmp
# =========================================================
def ensure_dataset():
    if not os.path.exists(LOCAL_PATH):
        print("🔥 Downloading dataset from Backblaze...")
...
# =========================================================
# CONNECT TO DUCKDB — CREATE ENRICHED VIEW
# =========================================================
...
con.execute(f"""
    ...
    SELECT
        *,
        ...
        -- Extract hour safely
        --------------------------------------
        CAST(SPLIT_PART(crash_time, ':', 1) AS INTEGER) AS hour,

        --------------------------------------
        -- Total Injuries (same as your CSV)
        --------------------------------------
        (
        ...
        --------------------------------------
        -- Severity formula (same as your CSV)
        --------------------------------------
        (
            number_of_persons_injured +
            number_of_pedestrians_injured +
...
def build_where(borough, year, vehicle, factor, injury, query):
    ...
    if query:
        q = query.lower()
        filters.append(
            f"(lower(borough) LIKE '%{q}%' OR lower(on_street_name) LIKE '%{q}%')"
        )
...
def update(n, borough, year, vehicle, factor, injury, query):
    ...
    # ------------------ GRAPH 1 ---------------------
    df1 = q(f"""
        SELECT crash_year, borough, SUM(total_injuries) AS injuries
    ...
    fig6 = px.bar(df6, x="borough", y="count", color="person_sex",
                  barmode="group",
                  title="Gender Breakdown by Borough")
    ...
    fig8 = px.density_heatmap(df8, x="hour", y="vehicle_category", z="sev",
                              title="Severity Heatmap by Hour & Vehicle Category")
    ...
    fig9 = px.bar(df9, x="on_street_name", y="sev",
                  title="Top 15 Most Severe Streets")
