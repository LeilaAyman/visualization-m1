# ============================================
# main.py — FINAL RENDER VERSION (DuckDB + Local Cache + FIXED COLUMNS)
# ============================================
...
# We CREATE a VIEW with all computed columns.
con.execute(f"""
    CREATE VIEW collisions AS
    SELECT
        *,
        -- Extract hour
        CAST(SPLIT_PART(crash_time, ':', 1) AS INT) AS hour,
...
def build_where(borough, year, vehicle, factor, injury, query):
    ...
    if factor:  f.append(f"contributing_factor_combined IN ({','.join(repr(x) for x in factor)})")
    if injury:  f.append(f"person_injury IN ({','.join(repr(i) for i in injury)})")
    if query:
        qtxt = query.lower()
        f.append(f"(lower(borough) LIKE '%{qtxt}%' OR lower(on_street_name) LIKE '%{qtxt}%')")
...
def update(_, borough, year, vehicle, factor, injury, query):
    ...
    """)
    fig7 = px.line(df7.melt(id_vars="hour", var_name="type", value_name="avg"),
                   x="hour", y="avg", color="type")
    ...
    # 9
    df9 = q(f"""
        SELECT on_street_name, SUM(severity) AS sev
        FROM collisions {where}
